...
def index():
    ...
    if sort==1:
        ...
        #按照加精的时间倒叙排序(涉及到两个模型张表）分为加完精，未加精
...
def add_comment():
    ...
    if form.validate():
        ...
        if post:
            ...
            return restful.params_error("没有这边帖子")
    else:
...
class SignupView(views.MethodView):
    ...
    def post(self):
        ...
        if form.validate():
            ...
            user=FrontUer(telephone=telephone,username=username,password=password)
            db.session.add(user)
            db.session.commit()
            return  restful.success()
        else:
...
class SigninView(views.MethodView):
    ...
    def post(self):
        ...
        if form .validate():
            ...
            user=FrontUer.query.filter_by(telephone=telephone).first()
